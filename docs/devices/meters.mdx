...
Die aufgelisteten Geräte können als `meter` für verschiedene Zwecke eingebunden werden. Um die Ströme am Netzanschluss `grid` zu erhalten, die PV Leistung `pv`, die Hausbatterie `battery`, externe Geraete mit eigener Ueberschussregelung `aux` oder die Ladeleistung des EV `charge`.
